...
class SimulationVerifier:
    ...
    def verify(
        ...
        feeder_capacity_kw = sim_input.system_constraints.get("feeder_capacity_kw")
        if feeder_capacity_kw is not None:
            planned_by_time: Dict[TimeStep, float] = {}
            for resource_dispatch in dispatched.values():
                for time_step, power_kw in resource_dispatch.items():
                    planned_by_time[time_step] = planned_by_time.get(time_step, 0.0) + power_kw
            for time_step, power_kw in planned_by_time.items():
                if power_kw > float(feeder_capacity_kw) + 1e-9:
                    constraint_violations += 1
                    violations.append(
                        f"Dispatch exceeded feeder capacity {feeder_capacity_kw} kW "
                        f"at {time_step} with {power_kw} kW"
                    )
