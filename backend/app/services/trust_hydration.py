"""Bridge from an EV model client to optimizer context["trust_data"].

This module provides the batch hydration layer that takes a Scenario's resources,
runs AI/ML inference for each resource through the shared client contract and
...
class EVPrediction(Protocol):
    trust_state: TrustState


class EVModelClient(Protocol):
...
def hydrate_trust_context(
    ...
    Args:
        client: An initialized offline EV model client.
